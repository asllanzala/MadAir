    ...
    private BluetoothGattCallback mBluetoothGattCallback = new BluetoothGattCallback() {
        ...
        public void onConnectionStateChange(BluetoothGatt gatt, int status, int newState) {
            ...
            if (newState == BluetoothProfile.STATE_CONNECTED) {
                if (status == 0) {
                    ...
                    LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "Connected to GATT server." + gatt.getDevice().getAddress());
                    // Attempts to discover services after successful connection.
                    LogUtil.log(LogUtil.LogLevel.INFO, TAG, "Attempting to start service discovery:" + gatt
                            .discoverServices());
                ...
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "Disconnected from GATT server." + gatt.getDevice().getAddress());
                broadcastUpdate(intentAction, gatt.getDevice().getAddress(), gatt.getDevice().getName());
                gatt.close();
            }
        }
        ...
        public void onServicesDiscovered(BluetoothGatt gatt, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onServicesDiscovered");
                broadcastUpdate(ACTION_GATT_SERVICES_DISCOVERED, gatt.getDevice().getAddress(), gatt.getDevice().getName());
            } else {
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onServicesDiscovered received: " + status);
            }
        }
        ...
        public void onCharacteristicRead(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onCharacteristicRead "
        ...
        public void onCharacteristicWrite(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic, int status) {
            LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onCharacteristicWrite "
                    + characteristic.getUuid()
                    + " value " + Arrays.toString(characteristic.getValue()));
        ...
        public void onDescriptorWrite(BluetoothGatt gatt, BluetoothGattDescriptor descriptor, int status) {
            if (status == BluetoothGatt.GATT_SUCCESS) {
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onDescriptorWrite");
        ...
        public void onCharacteristicChanged(BluetoothGatt gatt, BluetoothGattCharacteristic characteristic) {
            LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "onCharacteristicChanged:" + Arrays.toString(characteristic.getValue()));
            ...
            if (BluetoothDevice.ACTION_BOND_STATE_CHANGED.equals(action)) {
                ...
                switch (device.getBondState()) {
                    case BluetoothDevice.BOND_BONDING:
                        LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "BOND_BONDING");
                        break;
                    case BluetoothDevice.BOND_BONDED:
                        LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "BOND_BONDED");
...
//                        }
                    ...
                    case BluetoothDevice.BOND_NONE:
                        LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "BOND_NONE");
            ...
            if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                ...
                switch (state) {
                    case BluetoothAdapter.STATE_OFF:
                        LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "Phone bluetooth off.");
                        broadcastUpdate(ACTION_PHONE_BLUETOOTH_OFF, "", "");
                        break;
                    case BluetoothAdapter.STATE_ON:
                        LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "Phone bluetooth on.");
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            while (connectedIterator.hasNext()) {
                ...
                if (bluetoothGatt != null) {
                    ...
                    bluetoothGatt.close();
                    LogUtil.log(LogUtil.LogLevel.INFO, TAG, "bluetoothGatt disconnect:" + bluetoothGatt.getDevice().getAddress());
                }
            }
    ...
    private final BroadcastReceiver mClearSameTypeDeviceReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ACTION_CLEAR_SAME_TYPE_DEVICE.equals(action)) {
                ...
                mBluetoothGattHashMap.clear();
                LogUtil.log(LogUtil.LogLevel.ERROR, TAG, "mBluetoothGattHashMap.put:");
