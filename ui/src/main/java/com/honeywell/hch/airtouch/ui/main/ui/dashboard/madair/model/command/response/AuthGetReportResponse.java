...
public class AuthGetReportResponse extends ResponseCommand {
    ...
    public AuthGetReportResponse(byte[] data) {
        super(data);
        ...
        mType = data[1];
        mBody = new byte[12];
        System.arraycopy(data, 2, mBody, 0, 12);
        mCrc = new byte[] {data[14], data[15]};

        // parse body
        mTimeStamp = new byte[] {data[2], data[3], data[4], data[5]};
        mBatteryPercent = convertNegative(data[6]);
        mBatteryRemain = convertNegative(data[7]);
        mFilter = new byte[] {data[8], data[9]};
        mMotorSpeed = data[10];
        mBreathFreq = data[11];
        mAlarmInfo = data[12];
